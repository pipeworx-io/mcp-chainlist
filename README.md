...
`tools/list` at `https://gateway.pipeworx.io/chainlist/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/chainlist_list_chains \
  -H 'Content-Type: application/json' \
  -d '{"testnet":false,"active":true}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/chainlist_list_chains`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
